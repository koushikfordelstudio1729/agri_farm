...
class Validators {
    ...
    static phone(phoneNumber, countryCode) {
        try {
            const parsed = (0, libphonenumber_js_1.parsePhoneNumber)(phoneNumber, countryCode);
    ...
    static password(password, options = {}) {
        const { minLength = 8, maxLength = 128, requireUppercase = true, requireLowercase = true, requireNumbers = true, requireSpecialChars = true, } = options;
        const errors = [];
    ...
    static dateRange(startDate, endDate, maxRange) {
    ...
    static fileSize(sizeInBytes, maxSizeInMB) {
    ...
    static postalCode(postalCode, countryCode) {
        const patterns = {
            US: /^\d{5}(-\d{4})?$/,
    ...
    static age(birthDate, minAge, maxAge) {
    ...
    static sanitizeString(input, options = {}) {
        const { maxLength, allowedChars, removeHtml = false, trim = true, } = options;
        let result = input;
        if (trim) {
            result = result.trim();
        }
    ...
    static businessRules = {
        ...
        harvestDate(plantingDate, harvestDate, cropType) {
            const growthPeriods = {
                rice: { min: 90, max: 180 },
            ...
            if (!period)
                return true; // Unknown crop type, allow any reasonable date
            const diffInDays = Math.floor((harvestDate.getTime() - plantingDate.getTime()) / (1000 * 60 * 60 * 24));
            return diffInDays >= period.min && diffInDays <= period.max;
        },
        weatherCondition(temperature, humidity) {
            return (temperature >= -50 && temperature <= 60 && // Celsius
                humidity >= 0 && humidity <= 100);
        },
        marketPrice(price, currency) {
            if (price < 0)
                return false;
            const maxPrices = {
                USD: 10000,
