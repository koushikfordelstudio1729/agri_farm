...
const plantImageSchema = new mongoose_1.Schema({
    id: {
        type: String,
        required: true,
    },
...
const diagnosisResultSchema = new mongoose_1.Schema({
    diseaseId: {
...
const diagnosisRequestSchema = new mongoose_1.Schema({
    cropId: {
        type: mongoose_1.Schema.Types.ObjectId,
        ref: 'Crop',
        required: true,
...
const diagnosisSchema = new mongoose_1.Schema({
    ...
    expertReview: {
        ...
        reviewedAt: Date,
        approved: Boolean,
        modifications: mongoose_1.Schema.Types.Mixed,
        notes: String,
    },
    processingTime: {
        type: Number,
...
// Instance methods
diagnosisSchema.methods.updateStatus = async function (status) {
    this.status = status;
    await this.save();
};
diagnosisSchema.methods.addResults = async function (results) {
    this.results = results;
    this.confidence = results.length > 0 ? Math.max(...results.map(r => r.confidence)) : 0;
    await this.save();
};
diagnosisSchema.methods.markAsCompleted = async function (results, processingTime) {
...
diagnosisSchema.methods.submitFeedback = async function (rating, comment, isHelpful) {
    this.feedback = {
        rating,
...
diagnosisSchema.methods.toPublicJSON = function () {
...
// Static methods
diagnosisSchema.statics.findByUserId = async function (userId) {
    return this.find({ userId }).sort({ createdAt: -1 });
};
diagnosisSchema.statics.findPending = async function () {
    return this.find({ status: 'pending' }).sort({ createdAt: 1 });
};
diagnosisSchema.statics.findByStatus = async function (status) {
    return this.find({ status }).sort({ createdAt: -1 });
};
diagnosisSchema.statics.getStats = async function (dateRange) {
...
exports.Diagnosis = mongoose_1.default.model('Diagnosis', diagnosisSchema);
