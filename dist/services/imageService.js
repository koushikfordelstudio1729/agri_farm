...
class ImageService {
    ...
    static async processImage(buffer, options = {}) {
        try {
            const { width, height, quality = 85, format = 'jpeg', fit = 'cover', crop = 'center', sharpen = false, normalize = false, } = options;
            ...
            logger_1.logger.info('Image processed successfully', {
            ...
            logger_1.logger.error('Image processing failed', error instanceof Error ? error : new Error(String(error)));
            throw new errors_1.FileUploadError('Image processing failed');
        }
    }
    ...
    static async addWatermark(buffer, watermarkOptions) {
        try {
        ...
        `;
            ...
            logger_1.logger.error('Watermark application failed', error instanceof Error ? error : new Error(String(error)));
            throw new errors_1.FileUploadError('Watermark application failed');
        }
    }
    static getSharpGravity(position) {
        const gravityMap = {
            'top-left': 'northwest',
            'top-center': 'north',
            'top-right': 'northeast',
    ...
    static async uploadToCloudinary(buffer, options = {}) {
        if (!this.isCloudinaryConfigured()) {
            throw new errors_1.ExternalServiceError('Cloudinary', 'Cloudinary not configured');
        }
        try {
            const { folder = 'agri-farm', publicId, transformation = [], tags = [], } = options;
            const uploadOptions = {
                folder,
            ...
            logger_1.logger.error('Cloudinary upload failed', error instanceof Error ? error : new Error(String(error)));
            throw new errors_1.ExternalServiceError('Cloudinary', 'Image upload failed', error);
        }
    }
    static generateCloudinaryThumbnailUrl(publicId) {
    ...
    static async deleteFromCloudinary(publicId) {
        ...
        try {
            ...
            logger_1.logger.error('Cloudinary deletion failed', error instanceof Error ? error : new Error(String(error)));
            throw new errors_1.ExternalServiceError('Cloudinary', 'Image deletion failed', error);
        }
    }
    static async getImageInfo(buffer) {
        try {
            ...
            logger_1.logger.error('Failed to get image info', error instanceof Error ? error : new Error(String(error)));
            throw new errors_1.FileUploadError('Failed to analyze image');
        }
    }
    static async validateImage(buffer, options = {}) {
        ...
        try {
            ...
            const { maxWidth, maxHeight, minWidth = 1, minHeight = 1, maxSize = 10 * 1024 * 1024, // 10MB
            allowedFormats = ['jpeg', 'jpg', 'png', 'webp', 'gif'], } = options;
            // Size validation
    ...
    static async processUploadedImages(files, options = {}) {
        const { generateThumbnails = true, uploadToCloud = true, processOptions = {}, thumbnailOptions = {}, } = options;
        const results = [];
        for (const file of files) {
            try {
                ...
                if (!validation.isValid) {
                    logger_1.logger.warn('Image validation failed', {
